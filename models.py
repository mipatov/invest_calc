...
class MacroMLModel(Model):
    ...
    #     prediction market
        ...
        start_quarter_date = datetime.datetime(
            year=market_quarter_df.iloc[-1].year, month=get_quarter_months(start_quarter_num)[0], day=1)
        ...
        start_price = market_data.loc[datetime.datetime.strftime(
            start_quarter_date, '%Y-%m-%d')][0]

        print('Quarter_data :', start_quarter_num,
              start_quarter_date, start_price)

        Q_period = ceil(period/3)+1
        Q_dt = [add_months(start_quarter_date, 3*i)
                for i in range(1, Q_period+1)]
